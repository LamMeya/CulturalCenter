...
async def list_teams(
    ...
    # 按 user_id 过滤：只返回当前用户加入的团队
    if user_id:
        if user.id != user_id:
            raise HTTPException(status_code=403, detail="无权查看他人团队")
        q = q.join(Team.members).filter(TeamMember.user_id == user_id).distinct()
    if search:
...
@router.post("")
async def create_team(body: CreateTeamBody, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    db.flush()
    # 创建者自动成为团长
    member = TeamMember(user_id=user.id, team_id=team.id, role=TeamRole.LEADER)
    db.add(member)
    db.commit()
    db.refresh(team)
...
async def join_team(team_id: int, body: JoinTeamBody, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    team = db.query(Team).filter_by(id=team_id, is_active=True).first()
    if not team:
        raise HTTPException(status_code=404, detail="团队不存在")
    if len(team.members) >= team.max_members:
        raise HTTPException(status_code=400, detail="团队已满员")
    existing = db.query(TeamMember).filter_by(user_id=user.id, team_id=team_id).first()
    if existing:
        raise HTTPException(status_code=400, detail="已加入该团队")
    member = TeamMember(user_id=user.id, team_id=team_id, role=TeamRole.MEMBER)
    db.add(member)
    db.commit()
    db.refresh(team)
    return _team_to_dict(team)
...
async def leave_team(team_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    member = db.query(TeamMember).filter_by(user_id=user.id, team_id=team_id).first()
    if not member:
